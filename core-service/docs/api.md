...
본인 계정을 탈퇴 처리합니다 (`status`를 `WITHDRAWN`으로 변경하는 soft delete, 데이터를 즉시 삭제하지 않습니다). 처리 후 보유한 모든 `refreshToken`이 즉시 폐기되어 다른 기기의 세션도 함께 로그아웃됩니다. 탈퇴한 계정은 이후 로그인할 수 없습니다.

로컬 계정(이메일/비밀번호)은 본인 확인을 위해 비밀번호가 필요합니다. 소셜 로그인 전용 계정은 body 없이 요청하면 됩니다.

**Request Body** (로컬 계정만 필요)

| 필드 | 타입 | 제약 |
|---|---|---|
| password | string | 로컬 계정인 경우 필수 |
...
**에러**
- `401` — 인증 실패, 또는 (로컬 계정인데) 비밀번호가 없거나 일치하지 않음
